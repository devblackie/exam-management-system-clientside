...
// Simple hash for email privacy - don't send raw emails to GA
function hashEmail(email: string): string {
  ...
  return `lead_${Math.abs(hash).toString(16)}`;
}
...
export function track(event: AnalyticsEvent, props?: EventProps): void {
  ...
  if (process.env.NODE_ENV === "development") {
    console.log(`[Analytics:${branding.devName}] ${event}`, safeProps);
  }
}

export function trackEmailGateShown(): void {
  track("demo_email_gate_shown");
}

export function trackEmailSubmitted(email: string): void {
  track("demo_email_submitted", { email_hash: hashEmail(email) });
}

export function trackScreenSwitch(screen: string): void {
  track("demo_screen_switched", { screen });
}

export function trackCMSDownload(): void {
  track("demo_cms_download");
}

export function trackTalkToTeam(): void {
  track("demo_talk_to_team");
}
...
export function trackSignupFormViewed(source?: string): void {
  track("signup_form_viewed", { source: source || "direct" });
}
...
export function trackSignupFormSubmitted(
  ...
  track("signup_form_submitted", { studentCount, source: source || "direct" });
}