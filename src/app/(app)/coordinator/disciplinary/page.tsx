...
import {
  listDisciplinaryCases, raiseCase, recordOutcome, reinstateStudent, recordAppeal,
  GROUNDS_LABELS, OUTCOME_LABELS,
  type DisciplinaryCase, type DisciplinaryGrounds, type DisciplinaryOutcome,
  type RecordOutcomePayload, type ListCasesParams,
} from "@/api/disciplinaryApi";
import {
  AlertTriangle, ShieldAlert, Search, Plus, CheckCircle2,
  Clock, XCircle, AlertCircle, Gavel, RefreshCcw,
  ChevronLeft, ChevronRight, Loader2, User, ChevronDown, Scale,
} from "lucide-react";

// ── Outcome pill ──────────────────────────────────────────────────────────────
function OutcomePill({ outcome }: { outcome: DisciplinaryOutcome }) {
  const styles: Record<DisciplinaryOutcome, { bg: string; text: string; icon: React.ReactNode }> = {
    PENDING:      { bg: "bg-amber-100",    text: "text-amber-800",   icon: <Clock size={10} /> },
    SENT_HOME:    { bg: "bg-red-100",      text: "text-red-800",     icon: <AlertCircle size={10} /> },
    WARNING:      { bg: "bg-orange-100",   text: "text-orange-800",  icon: <AlertTriangle size={10} /> },
    REINSTATED:   { bg: "bg-emerald-100",  text: "text-emerald-800", icon: <CheckCircle2 size={10} /> },
    DISCONTINUED: { bg: "bg-slate-800",    text: "text-slate-100",   icon: <XCircle size={10} /> },
    DISMISSED:    { bg: "bg-slate-100",    text: "text-slate-600",   icon: <CheckCircle2 size={10} /> },
  };
  const s = styles[outcome] ?? styles.PENDING;
  return (
    <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider ${s.bg} ${s.text}`}>
      {s.icon} {OUTCOME_LABELS[outcome] ?? outcome}
    </span>
  );
}

// ── Shared tokens (matching coordinator page) ─────────────────────────────────
const inputCls = [
  "w-full bg-white border border-green-darkest/10 rounded-lg",
  "py-2.5 px-4 text-xs font-mono text-green-darkest",
  "placeholder:text-slate-300 outline-none",
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-6">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg border border-green-darkest/5 animate-in fade-in zoom-in-95 duration-200">
        <div className="bg-green-darkest rounded-t-2xl px-8 py-5 flex items-center justify-between">
          <div>
            <p className="text-yellow-gold text-[9px] font-black uppercase tracking-[0.3em] mb-1">{subtitle}</p>
...
export default function DisciplinaryPage() {
  ...
  const suspended  = cases.filter(c => c.outcome === "SENT_HOME").length;
  const resolved   = cases.filter(c => ["DISMISSED","WARNING","REINSTATED","DISCONTINUED"].includes(c.outcome)).length;

  const ribbonStats = [
    { title: "Total Cases",  value: loading ? "..." : total.toString(),     icon: <Scale size={24} /> },
    { title: "Pending",      value: loading ? "..." : pending.toString(),    icon: <Clock size={24} /> },
    { title: "Suspended",    value: loading ? "..." : suspended.toString(),  icon: <ShieldAlert size={24} /> },
    { title: "Resolved",     value: loading ? "..." : resolved.toString(),   icon: <CheckCircle2 size={24} /> },
  ];

  // Handlers — identical logic to original, unchanged
  const handleRaise = async () => {
    if (!regNo.trim()) return addToast("Enter the student registration number.", "error");
    if (description.trim().length < 10) return addToast("Description must be at least 10 characters.", "error");
    setSubmitting(true);
    try {
      const students = await searchStudents(regNo.trim());
      const student  = students?.[0];
      if (!student) { addToast("No student found with that registration number.", "error"); return; }
      const result = await raiseCase({ studentId: student._id, grounds, description: description.trim(), hearingDate: hearingDate || undefined });
      addToast(result.message, "success");
      setShowRaise(false); setRegNo(""); setGrounds("misconduct"); setDescription(""); setHearingDate("");
      fetchCases();
    } catch (err: unknown) {
      addToast((err as { response?: { data?: { message?: string } } })?.response?.data?.message ?? "Failed to file case.", "error");
  ...
  const handleOutcome = async () => {
    ...
    setSubmitting(true);
    try {
      const payload: RecordOutcomePayload = { outcome: ocOutcome, outcomeNotes: ocNotes || undefined, hearingDate: ocHearingDate || undefined, suspensionStart: ocSuspStart || undefined, suspensionEnd: ocSuspEnd || undefined };
      const result = await recordOutcome(activeOutcome._id, payload);
      addToast(result.message, "success");
      setActiveOutcome(null); fetchCases();
    } catch (err: unknown) {
      addToast((err as { response?: { data?: { message?: string } } })?.response?.data?.message ?? "Failed to record outcome.", "error");
    } finally { setSubmitting(false); }
  };

  const handleReinstate = async (caseId: string, studentName: string) => {
    if (!window.confirm(`Reinstate ${studentName}? Their suspension will be lifted immediately.`)) return;
    try {
      const result = await reinstateStudent(caseId, { notes: "Reinstated via admin dashboard." });
      addToast(result.message, "success"); fetchCases();
    } catch (err: unknown) {
      addToast((err as { response?: { data?: { message?: string } } })?.response?.data?.message ?? "Failed to reinstate student.", "error");
    }
  };
  ...
  const handleAppeal = async () => {
    ...
    setSubmitting(true);
    try {
      const result = await recordAppeal(activeAppeal._id, { appealOutcome: apOutcome, appealNotes: apNotes || undefined });
      addToast(result.message, "success");
      setActiveAppeal(null); fetchCases();
    } catch (err: unknown) {
      addToast((err as { response?: { data?: { message?: string } } })?.response?.data?.message ?? "Failed to record appeal.", "error");
    } finally { setSubmitting(false); }
  };

  const filtered    = cases.filter(c => !search || c.student.name.toLowerCase().includes(search.toLowerCase()) || c.student.regNo.toLowerCase().includes(search.toLowerCase()));
  const totalPages  = Math.ceil(total / PAGE_SIZE);
    ...
    <ProtectedRoute allowed={["coordinator", "admin"]}>
      <div className="max-w-8xl lg:ml-48 mt-14 animate-in fade-in slide-in-from-bottom-4 duration-700">
        <div className="bg-[#F8F9FA] min-h-screen rounded shadow-2xl p-10 relative overflow-hidden">
          ...
          <PageHeader title="Disciplinary" highlightedTitle="Registry" systemLabel="Conduct Management Portal" />
                  ...
                  <div key={stat.title} className="flex-1 px-10 relative group border-r border-green-darkest/[0.06] last:border-r-0">
                    <div className="relative">
                      ...
                      <div className="flex items-baseline gap-1">
                        {stat.value === "..." ? (
                          <span className="h-10 w-16 bg-slate-200 animate-pulse rounded-lg inline-block" />
                        ) : (
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
            <div className="relative group max-w-sm w-full">
              ...
              <input
                ...
                value={search}
                onChange={e => setSearch(e.target.value)}
                className="w-full text-slate-500 bg-white border border-green-darkest/10 rounded-lg py-2.5 pl-11 pr-4 text-xs font-mono outline-none focus:ring-2 focus:ring-yellow-gold/20"
              />
            </div>
            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                <select
                  value={outcomeFilter}
                  onChange={e => { setOutcomeFilter(e.target.value as DisciplinaryOutcome | ""); setPage(1); }}
                  className="appearance-none bg-white border border-green-darkest/10 rounded-lg py-2.5 pl-4 pr-8 text-xs font-mono text-green-darkest outline-none focus:ring-2 focus:ring-yellow-gold/20"
                >
                  <option value="">All Outcomes</option>
                  {(Object.entries(OUTCOME_LABELS) as [DisciplinaryOutcome, string][]).map(([val, label]) => (
          ...
          {/* ── Table — matching coordinator card style ─────────────────────── */}
          <div className="bg-white border border-green-darkest/5 rounded-xl shadow-sm overflow-hidden">
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-green-darkest/5 bg-slate-50/50">
                  {["Student", "Grounds", "Outcome", "Hearing Date", "Raised By", "Actions"].map(h => (
                    <th key={h} className="px-6 py-4 text-left text-[9px] font-black uppercase tracking-[0.2em] text-slate-400">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-green-darkest/5">
                {loading ? (
                  <tr><td colSpan={6} className="py-20 text-center"><Loader2 size={20} className="animate-spin text-slate-300 mx-auto" /></td></tr>
                ) : filtered.length === 0 ? (
                  <tr><td colSpan={6} className="py-20 text-center text-xs font-mono text-slate-300 uppercase tracking-widest">No cases found</td></tr>
                ) : filtered.map(c => (
                  <tr key={c._id} className="hover:bg-slate-50/80 transition-colors group/row">
                    <td className="px-6 py-3">
                      <div className="flex items-center gap-3">
                        <div className="h-8 w-8 rounded-lg bg-green-darkest/5 flex items-center justify-center">
                          <User size={14} className="text-green-darkest/40" />
                        ...
                        <div>
                          <p className="text-[11px] font-bold text-green-darkest">{c.student.name}</p>
                          <p className="text-[9px] font-mono text-slate-400">{c.student.regNo} · Y{c.student.currentYearOfStudy}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-3 text-[10px] font-bold text-slate-600">{GROUNDS_LABELS[c.grounds] ?? c.grounds}</td>
                    <td className="px-6 py-3">
                      <OutcomePill outcome={c.outcome} />
                      {c.appealed && c.appealOutcome && (
                        <span className="ml-2 text-[8px] font-black text-indigo-600 uppercase tracking-wider">Appeal {c.appealOutcome}</span>
                      )}
                    </td>
                    <td className="px-6 py-3 text-[10px] font-mono text-slate-400">
                      {c.hearingDate ? new Date(c.hearingDate).toLocaleDateString("en-KE", { day: "2-digit", month: "short", year: "numeric" }) : "—"}
                    </td>
                    <td className="px-6 py-3 text-[10px] font-mono text-slate-500">{c.raisedBy.name}</td>
                    <td className="px-6 py-3">
                      <div className="flex items-center gap-2">
                        {isAdmin && c.outcome === "PENDING" && (
                          <button
                            onClick={() => { setActiveOutcome(c); setOcOutcome("WARNING"); setOcNotes(""); setOcHearingDate(""); setOcSuspStart(""); setOcSuspEnd(""); }}
                            className="flex items-center gap-1.5 px-3 py-1.5 bg-green-darkest text-white text-[9px] font-black uppercase tracking-wider rounded-lg hover:bg-green-800 transition-all"
                          >
                            <Gavel size={11} className="text-yellow-gold" /> Outcome
                          </button>
                        )}
                        {isAdmin && c.outcome === "SENT_HOME" && (
                          <button onClick={() => handleReinstate(c._id, c.student.name)}
                            className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-600 text-white text-[9px] font-black uppercase tracking-wider rounded-lg hover:bg-emerald-700 transition-all">
                            <RefreshCcw size={11} /> Reinstate
                          </button>
                        )}
                        {isAdmin && ["SENT_HOME","DISCONTINUED","WARNING"].includes(c.outcome) && !c.appealed && (
                          <button onClick={() => { setActiveAppeal(c); setApOutcome("DISMISSED"); setApNotes(""); }}
                            className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-600 text-white text-[9px] font-black uppercase tracking-wider rounded-lg hover:bg-indigo-700 transition-all">
                            <Scale size={11} /> Appeal
                          </button>
                        )}
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-4 mt-8">
              ...
              <span className="text-[10px] font-mono text-slate-400 uppercase tracking-widest">Page {page} of {totalPages}</span>
      ...
      {/* ── Raise Case Modal ──────────────────────────────────────────────────── */}
      {showRaise && (
        <Modal title="File Disciplinary Case" subtitle="Conduct Portal" onClose={() => setShowRaise(false)}>
          <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
            <p className="text-[10px] font-bold text-red-700 uppercase tracking-wider">⚠ Filing this case immediately suspends system access pending a hearing.</p>
          </div>
          <div>
            <label className={labelCls}>Student Registration Number</label>
            <input type="text" placeholder="E.g. E024-01-1234/2022" className={inputCls} value={regNo} onChange={e => setRegNo(e.target.value)} />
          </div>
          <div>
            <label className={labelCls}>Grounds</label>
            <select className={inputCls} value={grounds} onChange={e => setGrounds(e.target.value as DisciplinaryGrounds)}>
              {(Object.entries(GROUNDS_LABELS) as [DisciplinaryGrounds, string][]).map(([val, label]) => (
                <option key={val} value={val}>{label}</option>
              ))}
            </select>
          </div>
          <div>
            <label className={labelCls}>Incident Description (min. 10 characters)</label>
            <textarea rows={4} className={inputCls + " resize-none"} placeholder="Describe the incident in detail…" value={description} onChange={e => setDescription(e.target.value)} />
          </div>
          <div>
            <label className={labelCls}>Scheduled Hearing Date (optional)</label>
            <input type="date" className={inputCls} min="2000-01-01" value={hearingDate} onChange={e => setHearingDate(e.target.value)} />
          </div>
          <div className="flex gap-3 pt-2">
            <button onClick={() => setShowRaise(false)} className="flex-1 border border-green-darkest/10 text-green-darkest py-3 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-slate-50 transition-all">Cancel</button>
      ...
      {/* ── Outcome Modal ──────────────────────────────────────────────────────── */}
      {activeOutcome && (
        <Modal title={`Outcome — ${activeOutcome.student.name}`} subtitle="Hearing Result" onClose={() => setActiveOutcome(null)}>
          <div>
            <label className={labelCls}>Outcome</label>
            <select className={inputCls} value={ocOutcome} onChange={e => setOcOutcome(e.target.value as DisciplinaryOutcome)}>
              <option value="WARNING">Warning — student continues studies</option>
              <option value="SENT_HOME">Sent Home — suspension formalised (RP1D applied)</option>
              <option value="DISCONTINUED">Discontinued — per ENG.22</option>
          ...
          {ocOutcome === "SENT_HOME" && (
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className={labelCls}>Suspension Start</label>
                <input type="date" min="2000-01-01" className={inputCls} value={ocSuspStart} onChange={e => setOcSuspStart(e.target.value)} />
              </div>
              <div>
                <label className={labelCls}>Expected Return (optional)</label>
                <input type="date" min="2000-01-01" className={inputCls} value={ocSuspEnd} onChange={e => setOcSuspEnd(e.target.value)} />
              </div>
            </div>
          )}
          <div>
            <label className={labelCls}>Hearing Date</label>
            <input type="date" min="2000-01-01" className={inputCls} value={ocHearingDate} onChange={e => setOcHearingDate(e.target.value)} />
          </div>
          <div>
            <label className={labelCls}>Committee Notes</label>
            <textarea rows={3} className={inputCls + " resize-none"} placeholder="Record the committee's reasoning…" value={ocNotes} onChange={e => setOcNotes(e.target.value)} />
          </div>
          <div className="flex gap-3 pt-2">
            <button onClick={() => setActiveOutcome(null)} className="flex-1 border border-green-darkest/10 text-green-darkest py-3 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-slate-50 transition-all">Cancel</button>
            <button onClick={handleOutcome} disabled={submitting}
              className="flex-1 bg-green-darkest hover:bg-green-800 text-white py-3 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all disabled:opacity-50">
              {submitting ? <Loader2 size={14} className="animate-spin" /> : <Gavel size={14} className="text-yellow-gold" />}
              {submitting ? "Saving…" : "Record Outcome"}
            </button>
          </div>
        </Modal>
      ...
      {/* ── Appeal Modal ──────────────────────────────────────────────────────── */}
      {activeAppeal && (
        <Modal title={`Record Appeal — ${activeAppeal.student.name}`} subtitle="Appeal Registry" onClose={() => setActiveAppeal(null)}>
          <div className="p-4 bg-indigo-50 border border-indigo-200 rounded-lg">
            <p className="text-[10px] font-bold text-indigo-700 uppercase tracking-wider">Original outcome: {OUTCOME_LABELS[activeAppeal.outcome]}</p>
          </div>
          <div>
            <label className={labelCls}>Appeal Outcome</label>
            <select className={inputCls} value={apOutcome} onChange={e => setApOutcome(e.target.value as "UPHELD" | "DISMISSED")}>
              <option value="UPHELD">Upheld — student wins, automatically reinstated</option>
              <option value="DISMISSED">Dismissed — original outcome stands</option>
            </select>
          </div>
          <div>
            <label className={labelCls}>Notes</label>
            <textarea rows={3} className={inputCls + " resize-none"} placeholder="Grounds for appeal decision…" value={apNotes} onChange={e => setApNotes(e.target.value)} />
          </div>
          <div className="flex gap-3 pt-2">
            <button onClick={() => setActiveAppeal(null)} className="flex-1 border border-green-darkest/10 text-green-darkest py-3 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-slate-50 transition-all">Cancel</button>
