  ...
  const fetchSummary = useCallback(async () => {
    ...
    try {
      ...
      if (status === 404) {
        ...
        addToast(
          "Failed to load billing data. Check your connection.",
          "error",
        );
        setSummary(null);
      }
    ...
    try {
      ...
      const msg =
        (err as { response?: { data?: { message?: string } } })?.response?.data
          ?.message ?? "Could not generate invoice. Check account status.";
      addToast(msg, "error");
    } finally {
      setGenerating(false);
    }
          ...
          <div className="bg-[#F8F9FA] min-h-[100vh] rounded-xl shadow-2xl p-10">
            ...
            <div className="grid grid-cols-4 gap-5 mb-6">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className="bg-white rounded-xl h-28 animate-pulse border border-green-darkest/5"
                />
              ))}
            </div>
            <div className="bg-white rounded-xl h-20 animate-pulse border border-green-darkest/5 mb-6" />
        ...
        <div className="max-w-8xl ml-40 my-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
          <div className="bg-[#F8F9FA] min-h-[100vh] rounded-xl shadow-2xl p-10 relative overflow-hidden">
            ...
            <div className="mt-20 flex flex-col items-center justify-center text-center max-w-md mx-auto">
              ...
              <h2 className="text-xl font-light text-green-darkest mb-3">
                Billing not configured
              </h2>
              <p className="text-sm text-slate-400 font-light leading-relaxed mb-8">
                No billing record found for this institution. This is usually
                resolved automatically on the next server restart after the{" "}
              ...
              <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl text-left w-full mb-8">
                <p className="text-[10px] font-black text-amber-700 uppercase tracking-wider mb-2">
                  Quick fix — run once in production:
                </p>
                <pre className="text-[10px] font-mono text-amber-800 whitespace-pre-wrap leading-relaxed">
                  {`node -e "
  require('dotenv').config();
        ...
        <div className="bg-[#F8F9FA] min-h-[100vh] rounded-xl shadow-2xl p-10 relative overflow-hidden">
          ...
          {tab === "overview" && (
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-xl border border-green-darkest/5 shadow-sm p-6">
                <div className="flex items-center justify-between mb-4">
                  <p className="text-[11px] font-black text-green-darkest uppercase tracking-wider">
                    Recent Invoices
                  </p>
                  <button
                    onClick={() => setTab("invoices")}
                ...
                {summary.recentInvoices.length === 0 ? (
                  <div className="py-10 text-center">
                    <p className="text-[11px] font-mono text-slate-300 uppercase tracking-widest mb-2">
                      No invoices yet
                    </p>
                    <p className="text-[10px] text-slate-300">
                      Click <span className="font-bold">Generate Invoice</span>{" "}
                  ...
                  <div className="divide-y divide-slate-100">
                    {summary.recentInvoices.map((inv) => {
                      ...
                      return (
                        ...
                        >
                          <div>
                            <p className="text-[11px] font-bold text-green-darkest font-mono">
                              {inv.invoiceNumber}
                            </p>
          ...
          {tab === "settings" && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              {/* Account status card */}
              <div className="bg-white rounded-xl border border-green-darkest/5 shadow-sm p-6 lg:col-span-2">
                <p className="text-[11px] font-black text-green-darkest uppercase tracking-wider mb-4">
                  Account Status
                </p>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
