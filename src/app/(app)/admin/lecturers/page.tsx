// clientside/src/app/(app)/admin/lecturers/page.tsx
"use client";
...
export default function LecturersPage() {
  ...
  useEffect(() => {
    const loadLecturers = async () => {
      try {
        const lecRes = await getLecturers();
        // Filter to ensure only lecturers (though API should return only lecturers)
        const lecturerOnly = lecRes.filter(
          (user) => user.role === "lecturer",
        ) as Lecturer[];
        setLecturers(lecturerOnly);
      } catch {
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      // Temporarily commented until API is implemented
      addToast(`Protocol Executed: Assignment successful`, "success");
      setSelectedUnits([]);
    } catch {
  ...
  return (
    <div className="max-w-9xl lg:ml-48 mt-10 px-2 animate-in fade-in duration-700">
          ...
          <div className="col-span-12 lg:col-span-5 flex flex-col gap-6">
            <div className="bg-white border border-green-darkest/5 rounded-xl p-8 shadow-sm">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-green-darkest/40">
                  Identity Selection
                </h3>
              </div>
              ...
              <div className="mt-8 p-4 bg-slate-50 rounded-lg border border-dashed border-slate-200">
                <p className="text-[9px] font-medium text-slate-400 leading-relaxed uppercase tracking-widest">
                  Assigning units grants full grading privileges and attendance
                  monitoring for the selected academic cycle.
                </p>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                {units.length === 0 ? (
                  <div className="col-span-2 text-center py-10 text-slate-400 text-xs">
                    No units available. Please add units to the template library
                    first.
                  </div>
                ) : (
                  units.map((unit) => (
                    ...
                    >
                      <div className="flex flex-col">
                        ...
                        <span
                          className={`text-[9px] font-medium uppercase truncate w-40 ${selectedUnits.includes(unit._id) ? "text-white/60" : "text-slate-400"}`}
                        >
                          {unit.name}
                        </span>
                      </div>
                      ...
                      />
                      {selectedUnits.includes(unit._id) && (
                        <CheckCircle size={14} className="text-yellow-gold" />
                      )}
                    </label>
                  ))
                )}
