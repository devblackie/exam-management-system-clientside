...
import { useCreateProgram } from "@/hooks/queries/usePrograms";
import { useInstitutionSettings } from "@/hooks/queries/useInstitutionSettings";
import { useToast } from "@/context/ToastContext";
import { getErrorMessage } from "@/lib/api";
...
const DEGREE_TYPES = ["BSc", "BEd", "BTech", "BEng", "BArch", "MBBS", "LLB", "BPharm", "Other"] as const;
const DURATIONS = [3, 4, 5, 6, 7] as const;

const inputStyle =
  "w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-700 text-sm outline-none focus:ring-2 focus:ring-yellow-gold/30 focus:border-yellow-gold/50 transition-all placeholder:text-slate-300";
const selectStyle = `${inputStyle} cursor-pointer`;
const labelStyle = "text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1.5 block";
...
export default function ProgramForm({ onClose, onSuccess }: ProgramFormProps) {
  ...
  const updateField = <K extends keyof FormData>(field: K, value: FormData[K]) => {
    setFormData((prev) => ({ ...prev, [field]: value }));
    if (field === "schoolCode") setFormData((prev) => ({ ...prev, departmentCode: "" }));
  };
  ...
  return (
    ...
    >
      ...
      <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
        ...
        <button onClick={onClose} className="p-1.5 hover:bg-slate-100 text-slate-400 rounded-lg transition-colors">
          <X size={16} />
        </button>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-6">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className={labelStyle}>School</label>
            <select className={selectStyle} value={formData.schoolCode} onChange={(e) => updateField("schoolCode", e.target.value)} required>
              <option value="">Select school...</option>
              {schools.map((s: School) => (
                <option key={s.code} value={s.code}>{s.name}</option>
              ))}
            ...
            {schools.length === 0 && (
              <p className="text-[10px] text-amber-600 mt-1">No schools configured. Set up schools first.</p>
            )}
          </div>

          <div>
            <label className={labelStyle}>Department</label>
            <select className={selectStyle} value={formData.departmentCode} onChange={(e) => updateField("departmentCode", e.target.value)} disabled={!formData.schoolCode} required>
              <option value="">Select department...</option>
              {departments.map((d: Department) => (
                <option key={d.code} value={d.code}>{d.name}</option>
              ))}
          ...
          <div>
            <label className={labelStyle}>Program Code</label>
            <input type="text" className={inputStyle} value={formData.code} onChange={(e) => updateField("code", e.target.value.toUpperCase())} placeholder="e.g., BSCE" maxLength={10} required />
            <p className="text-[9px] text-slate-400 mt-0.5">Unique identifier, max 10 chars</p>
          </div>

          <div>
            <label className={labelStyle}>Degree Type</label>
            <select className={selectStyle} value={formData.degreeType} onChange={(e) => updateField("degreeType", e.target.value)}>
              {DEGREE_TYPES.map((t) => (
                <option key={t} value={t}>{t}</option>
              ))}
            </select>
          ...
          <div>
            <label className={labelStyle}>Duration (Years)</label>
            <select className={selectStyle} value={formData.durationYears} onChange={(e) => updateField("durationYears", e.target.value)}>
              {DURATIONS.map((y) => (
          ...
          <div className="col-span-2">
            <label className={labelStyle}>Program Full Name</label>
            <input type="text" className={inputStyle} value={formData.name} onChange={(e) => updateField("name", e.target.value)} placeholder="e.g., Bachelor of Science in Civil Engineering" required />
          </div>

          <div className="col-span-2">
            <label className={labelStyle}>Description (Optional)</label>
            <textarea rows={2} className={`${inputStyle} resize-none`} value={formData.description} onChange={(e) => updateField("description", e.target.value)} placeholder="Brief description of the program..." />
          </div>
        </div>
        ...
        <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-slate-100">
          ...
          <button type="submit" disabled={createProgram.isPending} className="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded-lg shadow-sm transition-all disabled:opacity-50 flex items-center gap-2">
