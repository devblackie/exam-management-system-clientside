...
export default function PageHeader({
  ...
  // Derive display values — null means still checking
  const statusColor =
    isOnline === null
      ? "bg-slate-300"           // grey while checking
      : isOnline
      ? "bg-yellow-gold"         // gold when online
      : "bg-red-500";            // red when offline
    ...
    <header className="mb-8 w-full animate-in fade-in slide-in-from-top-4 duration-700">
      <div className="flex justify-between items-end border-b border-green-darkest/10 pb-2">
        ...
        <div className="flex flex-col">
          <div className="flex items-center gap-3 mb-1">
            <span
              className={`h-1 w-8 rounded-full transition-colors duration-500 ${statusColor}`}
            />
            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-green-darkest/40">
              {systemLabel}
            </span>
            ...
            <div className="text-right hidden sm:block border-l border-green-darkest/10 pl-6">
              <div className="flex items-center justify-end gap-2 mb-1">
                ...
                <span className="relative flex h-2 w-2">
                  ...
                  <span
                    className={`relative inline-flex rounded-full h-2 w-2 transition-colors duration-500 ${dotColor}`}
                  />
                </span>

                <p
                  className={`text-[9px] font-black uppercase tracking-[0.2em] transition-colors duration-500 ${statusTextColor}`}
                >
                  {statusText}
                </p>
              </div>
