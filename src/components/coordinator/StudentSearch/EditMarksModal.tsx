// clientside/src/components/coordinator/StudentSearch/EditMarksModal.tsx
"use client";
...
export default function EditMarksModal({ isOpen, onClose, student, editingMark, onSave, availableUnits, availableYears, settings }: EditMarksModalProps) {
  ...
  // Extract values from settings.ruleSet (nested) or fallback to defaults
  ...
  }, [isDirect, isSpecial, editingMark, settings, currentUnitType, examMode, catMax, assignmentMax, practicalMax, passMark]);
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                ...
                <div className="space-y-4">
                  <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
                    ...
                    {hasWorkshop && currentUnitType === "workshop" && (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <MarkInput name="cat1" label="CAT 1" defaultValue={editingMark?.cat1Raw} max={catMax} disabled={hasWorkshop && currentUnitType === "workshop"} />
                    <MarkInput name="cat2" label="CAT 2" defaultValue={editingMark?.cat2Raw} max={catMax} disabled={hasWorkshop && currentUnitType === "workshop"} />
                    <MarkInput name="cat3" label="CAT 3" defaultValue={editingMark?.cat3Raw} max={catMax} disabled={hasWorkshop && currentUnitType === "workshop"} />
                    <MarkInput name="assignment1" label="Assignments" defaultValue={editingMark?.assgnt1Raw} max={assignmentMax} disabled={hasWorkshop && currentUnitType === "workshop"} />
                    <MarkInput name="practicalRaw" label="Practical" defaultValue={editingMark?.practicalRaw} max={practicalMax} disabled={currentUnitType === "theory"} />
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">
                    Section B: Final Assessment (/{hasWorkshop && currentUnitType === "workshop" ? 60 : 70} Weighted)
                  </span>
                  <div className="mb-4">
                  ...
                  <div className="bg-blue-50/50 p-4 rounded-xl border border-blue-100">
                    <p className="text-[10px] text-blue-600/80 leading-tight">
                      Best {examMode === "mandatory_q1" ? "2" : "3"} others + Q1 are summed and scaled to {hasWorkshop && currentUnitType === "workshop" ? "60%" : "70%"}.
                    </p>
                  </div>
                </div>
              </div>
  ...
  return (
    <div className={`p-2 rounded-xl border transition-all ${
      ...
      <label className="text-[9px] font-black text-slate-400 mb-1 px-1 uppercase flex justify-between">
        <span>{label}</span>
        <span className={clamped ? "text-red-500" : "text-slate-300"}>/{max}</span>
      </label>
      <input
        name={name}
        type="number"
