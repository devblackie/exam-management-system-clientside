...
// Matching coordinator page input style exactly
const inputClass = [
...
export default function MarkManager({ units, programs, years }: MarkManagerProps) {
  ...
  const [confirmDelete, setConfirmDelete] = useState("");
  const [filters,       setFilters]       = useState<CleanupFilters>({
    unitCode: "", programId: "", academicYear: "",
  });
  ...
  const runCleanup = async () => {
    if (!filters.academicYear) { addToast("Please select an academic year", "error"); return; }
    if (confirmDelete !== "DELETE") return;
    try {
      const res = await maintenanceApi.bulkTrash(filters);
      addToast(res.message, "success");
  ...
  return (
    <div className="border border-green-darkest/5 rounded-xl bg-white shadow-sm overflow-hidden">

      {/* Tab bar — coordinator quick-access style */}
      <div className="flex bg-slate-50 border-b border-slate-100">
        <button
          onClick={() => setView("clean")}
          className={`flex-1 flex items-center justify-center gap-3 px-6 py-4 text-[10px] font-black uppercase tracking-widest transition-all ${
            view === "clean"
              ? "bg-white text-red-600 border-t-2 border-red-500 shadow-sm"
              : "text-slate-400 hover:text-slate-600 hover:bg-white/50"
          }`}
        >
          <Trash2 size={15} /> Cleanup Marks
        </button>
        <button
          onClick={() => setView("trash")}
          className={`flex-1 flex items-center justify-center gap-3 px-6 py-4 text-[10px] font-black uppercase tracking-widest transition-all ${
            view === "trash"
              ? "bg-white text-green-darkest border-t-2 border-green-darkest shadow-sm"
              : "text-slate-400 hover:text-slate-600 hover:bg-white/50"
          }`}
        >
          <Archive size={15} /> Trash Bin
        </button>
      </div>
      ...
      <div className="p-8">
        {view === "clean" ? (
          <div className="space-y-6">
            {/* Warning banner — coordinator danger-card style */}
            <div className="flex gap-3 p-5 bg-red-50 border border-red-200 rounded-xl">
              <ShieldAlert className="shrink-0 mt-0.5 text-red-500" size={18} />
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-red-700 mb-1">Dangerous Action</p>
                <p className="text-[11px] text-red-600 leading-relaxed">
                  Moving marks to trash hides them from transcripts and grade reports.
                  They can be restored from the Trash Bin tab.
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div>
                ...
                <select className={inputClass} value={filters.academicYear}
              ...
              <div>
                ...
                <select className={inputClass} onChange={e => setFilters({ ...filters, unitCode: e.target.value })}>
                  <option value="">All Units</option>
                  {units.map(u => <option key={u._id} value={u.code}>{u.code} — {u.name}</option>)}
                </select>
              </div>
              <div>
                <label className={labelCls}>Program</label>
                <select className={inputClass} onChange={e => setFilters({ ...filters, programId: e.target.value })}>
                  <option value="">All Programs</option>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5 items-end">
              <div className="md:col-span-2">
                <label className={labelCls}>Confirmation — type DELETE to unlock</label>
                <input
                  placeholder="Type DELETE to confirm"
                  className={inputClass + " text-center font-black uppercase tracking-widest"}
                  value={confirmDelete}
                  onChange={e => setConfirmDelete(e.target.value)}
                />
              </div>
              <button
                disabled={confirmDelete !== "DELETE"}
                onClick={runCleanup}
                className="w-full py-2.5 bg-red-600 hover:bg-red-700 disabled:bg-green-darkest/20 disabled:text-slate-300 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-sm"
              >
                Move to Bin
              </button>
            </div>
